...
import { Edit, Trash2, PlusCircle, Home } from 'lucide-react'; // <-- Added Home icon
import Modal from '../../components/common/Modal';
import { useAuthStore, IUser } from '../../stores/authStore';
import { useHotelStore } from '../../stores/hotelStore'; // <-- ADDED
...
  } = useManagementStore();
  
  // <-- ADDED: Get hotel data and logged-in user's info
  const { hotels, fetchHotels, getHotelNameById } = useHotelStore();
  const loggedInUser = useAuthStore((state) => state.user);
  const currentAdminId = loggedInUser?._id;

  // <-- ADDED: Check if the logged-in user is a Super Admin (no hotelId)
