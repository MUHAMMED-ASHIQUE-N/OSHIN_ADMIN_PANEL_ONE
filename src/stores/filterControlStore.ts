  ...
  fetchAvailableYears: async () => {
    ...
    try {
        ...
        const response = await axios.get<{ data: { years: number[] } }>(
            `${BASE_URL}/analytics/available-years`, // Use correct analytics endpoint URL
            config
        );
        ...
        if (fetchedYears && fetchedYears.length > 0) {
            ...
            // Select the most recent year if current isn't available or if it's the initial load default
